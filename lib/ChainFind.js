...
function ChainFind(Model, opts) {
	var chain = {
		...
		join: function(toTable, toFields, fromFields, fromTable) {
			if (!Array.isArray(opts.joins)){
				opts.joins = [];
			}
			if(fromTable === undefined){
				fromTable = opts.table;
			}
			if(!Array.isArray(toFields)){
				toFields = [toFields];
			}
			...
			opts.joins.push(newJoin);
			return this;
		},
		alias: function(alias) {
			join = _.last(opts.joins);
		...
		with: function() {
			...
			if(join) {
				...
				if (typeof args[0] === "object") {
					_.extend(join.conditions, args[0]);
				}
		...
		distinct_on: function() {
			opts.distinct_on = Array.prototype.slice.apply(arguments);
			return this;
    },
		order: function (property, order) {
			...
			if (property[0] === "-") {
				opts.order.push([ property.substr(1), "Z", opts.table]);
			} else {
				opts.order.push([ property, (order && order.toUpperCase() === "Z" ? "Z" : "A"), opts.table]);
			}
			return this;
		},
		count: function (cb) {
		...
		transform: function(func, args){
			opts.transform = {functionName: func, functionArgs: args};
		...
		asJson: function(cb) {
			...
			opts.driver.find(opts.only, opts.table, conditions, {
				...
				distinct_on: opts.distinct_on,
				format : "json"
			}, function (err, results) {
				if (err) {
					return cb(err);
				}
				if (results.length === 0 || results[0].json === null) {
					return cb(null, "[]");
				}

				cb(null, results[0].json);
