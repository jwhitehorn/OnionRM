...
function Model(opts) {
	...
	var model = function () {
	    ...
	    var data = arguments.length > 1 ? arguments : arguments[0], data2;
	...
	model.create = function () {
		...
		var createNext = function () {
			...
			Instances[idx] = createInstance(Instances[idx], {
				...
				Instances[idx].save({}, options, function (err) {
					if (err) {
						err.index = idx;
						err.instance = Instances[idx];
	...
	model.reconstitute = function(data, cb){
		var uid = opts.driver.uid + "/" + opts.table;
		for (var i = 0; i < opts.keys.length; i++) {
			uid += "/" + data[opts.keys[i]];
		}
