...
function Instance(Model, opts) {
    ...
    if(opts.partition){
      tableName = opts.partition.apply(instance);
    }
		opts.driver.insert(tableName, data, opts.keyProperties, function (save_err, info) {
			if (save_err) {
	...
	var saveInstanceExtra = function (cb) {
		...
		opts.driver.update(opts.extra_info.table, data, conditions, cb);
	};
	...
	Object.defineProperty(instance, "save", {
		value: function () {
			...
			if(typeof saveOptions.saveAssociations === "undefined" || saveOptions.saveAssociations === null){
				saveOptions.saveAssociations = false;
			}
			saveInstance(saveOptions, function (err, updatedInstance) {
				if (!cb) return;
				if (err) return cb(err);

				return cb(null, updatedInstance);
			});
	...
	Object.defineProperty(instance, "hasChanged", {
		value: function (propName) {
			if (propName != undefined) {
				return opts.changes.indexOf(propName) >= 0;
			}
		},
		enumerable: false,
		writable: true
